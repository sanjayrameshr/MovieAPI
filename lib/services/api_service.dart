...
class ApiService {
  ...
  static Future<List<dynamic>> searchMovies(String query) async {
    ...
    return _fetchMovieListData('/search/movie', query: query); // Query encoding is handled in _fetchMovieListData
  }
  ...
  /// This endpoint specifically returns comprehensive data including 'runtime',
  /// 'genres', 'tagline', 'homepage', etc.
  ...
  static Future<Map<String, dynamic>> fetchMovieDetails(int movieId) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        return data; // This response directly contains all the movie details, including runtime
      } else {
        String errorMessage;
        switch (response.statusCode) {
          case 401:
